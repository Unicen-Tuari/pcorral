...
{ 
	...
	{
		$sql = "INSERT INTO CLIENTE VALUES (NULL,'".$cliente['nombre']."','".$cliente['apellido']."','".$cliente['direccion']."','".$cliente['telefono']."','".$cliente['usuario']."','".$cliente['password']."','".$cliente['mail']."');";
		$resultado = $this->conn->prepare($sql);
		$resultado->execute();
		if (!$resultado)
		{
	...
	{
		$sql = "INSERT INTO COMPRAS VALUES ('".$queso['id_venta']."','".$queso['id_clientev']."','".$queso['importe']."');
	...
	public function consultaBusquedave($valor_principal,$valor_secundario,$tipo_busqueda)
	{
		...
		$sql =  "select c.id, cl.nombre, q.nombre as 'articulo', cl.apellido, c.importe, f.id_queso, f.precio, f.cantidad 
					from compras c
